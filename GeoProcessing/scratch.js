...
function process_shp_files(input_folder) {
    ...
    mapshaper.runCommands('-i combine-files snap ' + input_folder + '*0.shp -merge-layers force -simplify .01%  -o format=topojson zip precision=0.001 fix-geometry ' + input_folder, function(err) {
        if (err) {
            console.log(err);
        } else {
